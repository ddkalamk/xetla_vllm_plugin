...
ggml's parity construction. Run from ternsycl_vllm_plugin/: python tests/test_hadamard_cpu.py"""
import math
import os
import sys
...
src = open(os.path.join(HERE, "..", "ternsycl_vllm_plugin.py")).read()
start = src.index("# ---- Hadamard rotated basis")
end = src.index("class Timer")
ns = {"torch": torch, "os": os, "_ternsycl_prequant_meta": {},
      "_ternsycl_prequant_load_path": None}
...
class L:
    ternsycl_hadamard = None
    ternsycl_hadamard_inverse = None


torch.manual_seed(0)
H = ns["_ternsycl_hadamard_matrix"](1024, "cpu")
x = torch.randn(3, 5120)
signs = torch.where(torch.rand(5120) > 0.5, 1.0, -1.0)
l = L()
l.ternsycl_hadamard = (signs, 1024)
y = ns["_ternsycl_hadamard_fwd"](l, x)
ref = fwht((x * signs).reshape(-1, 1024)).reshape(3, 5120)
print("fwd max err", (y - ref).abs().max().item())
l2 = L()
l2.ternsycl_hadamard_inverse = (signs, 1024)
print("inv(fwd(x)) err", (ns["_ternsycl_hadamard_inv"](l2, y) - x).abs().max().item())
